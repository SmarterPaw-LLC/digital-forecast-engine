...
- **⚠ SQL TO RUN:** `supabase_v9_00_pnl_growth_saved_views.sql` — adds `user_profiles.pnl_growth_saved_views JSONB NOT NULL DEFAULT '{}'::jsonb`. Idempotent (`add column if not exists`). Same architecture as `forecast_saved_views` (v4.100) and `inventory_saved_views` (v4.166): Supabase canonical, localStorage cache for fast first paint.
- **What a "strategy" captures** — the 8 planning fields: `horizonMonths, targetShareEndPct, upliftPct, cpcInflationExp, maxMonthlyTacosPct, enforceTacosCeiling, annualMarketGrowthPct, portfolioRollup`. Runtime state (ASIN, masterId, lastResult, loading, kwSort) is deliberately EXCLUDED — a strategy is a reusable config, not a per-product session. So an "Aggressive push" strategy can be applied to Product A, then Product B, without changing meaning.
- **Chip bar above the settings grid** with the label `💾 Saved Strategies`. Each saved strategy renders as a segmented chip: click the name to APPLY, `↻` to overwrite with current settings, `✎` to rename, `✕` to delete. Active chip (matching current state exactly across all 8 fields) fills green with `●` prefix. Empty state shows a subtle italic hint pointing at the "💾 Save as strategy" button on the right.
- **`pnlGrowthSaveCurrentAsView()`** — prompts for a name, confirms overwrite if it exists, snapshots the current 8 fields, persists to both localStorage + Supabase. Audit log records `pnl_growth.view_save` / `_apply` / `_update` / `_rename` / `_delete`.
- **`pnlGrowthApplyView(name)`** — restores each of the 8 fields onto `pnlGrowthState`, re-renders the controls (via `pnlGrowthInit()`) so the input values reflect the loaded settings, AND auto-runs the model when an ASIN is currently selected. The whole point of applying a strategy is to see it evaluated immediately without an extra "▶ Run model" click.
- **Load path:** `pnlGrowthLoadSavedViewsFromDb()` fires from `init()` alongside the other view loaders (line 4752), so strategies sync across browsers/devices once the user signs in. localStorage cache is read synchronously at module init so chips paint on first visit even before Supabase resolves. If Supabase load succeeds and the Growth Model UI is already open, `pnlGrowthInit()` re-renders to pick up the fresh views.
- **Missing-column safety net:** if the user hasn't run the SQL migration yet, `pnlGrowthPersistSavedViews` detects the "column does not exist / not in schema cache" error and logs a warn pointing at the migration filename. Local cache still works for the session, just doesn't sync cross-device until the migration runs.
...
- **Bug #1 fix — richer labels via shared `entityDescriptor` + `buildAddRemoveLabel` helpers.** For every added/removed event, the label now includes the identifying data: keyword text (`"catnip treats"`), product-targeting expression, SKU, match type, placement (translated: `placementTop` → `Top-of-search`), percentage (for bidding adjustments), bid, daily budget (for campaigns), bidding strategy. Example: `Removed keyword: "catnip treats" · negativeExact · bid $0.75` instead of just `Removed Keyword`. For change events (bid up, budget down, state flip), the label appends the same entity descriptor so `Bid: $0.65 → $0.90` becomes `Bid: $0.65 → $0.90 · "catnip spray" · exact` — you can see WHICH keyword's bid moved without leaving the change log.
- **Bug #2 fix — trailing-timestamp first, always prompt on fallback.** Amazon's Bulk Sheet filenames are `bulk-{advertiser}-{startYYYYMMDD}-{endYYYYMMDD}-{unixMs}.xlsx`. The trailing 13-digit unixMs is when Amazon GENERATED the file (= when you downloaded it), which is the true snapshot date. The two YYYYMMDD dates are the user-selected REPORTING WINDOW (a filter, not a state date). v8.96 used the range-end, so a file downloaded on 9/21 with a 9/5→9/6 reporting range got dated 9/6. Worse: filenames that didn't match at all silently defaulted to `Date.now()` — Jason's "9/21" bug.
  - v8.97 parses the trailing `-(\d{13})\.xlsx$` pattern FIRST. If it looks like a plausible timestamp (past-2017, not in the future), uses it directly.
  - If that fails, falls back to the range-end pattern, then file.lastModified, then today.
  - **Whenever the trailing-timestamp path didn't fire**, the parser now shows a `prompt()` naming the extracted date, the source ("filename reporting-range end" / "file last-modified date"), and asks the user to confirm or edit before commit. So a filename mismatch can't silently produce a wrong-dated snapshot anymore.
- **Snapshot management panel** — new collapsible `📦 Uploaded snapshots — N · click to expand and manage` details block above the monthly bar chart. Lists every upload (newest first) with snapshot date · filename · row count · uploaded-ago · `🗑 Delete` button. `deleteBulkUpload(id, date)` global handler: confirm dialog, DELETE the upload row (FK cascade removes snapshot rows), invalidate the `amazonAdsBulkUploads` + `amazonAdsBulkSnapshotsByUpload` caches, re-run `renderPnlAdActivity`, refresh freshness probe, log `delete.amazon_ads_bulk_upload` to audit. Lets Jason clean up v8.96-era bad-date uploads without SQL.
- **How to clean up existing bad data:** open Amazon P&L → 🛠 Ad Activity Timeline → expand `📦 Uploaded snapshots` → click 🗑 Delete on any row with a suspect date (like today's date 2026-09-21 when the real download was 9/7). Then re-upload the actual Bulk Sheet — v8.97's parser will extract the correct date from the trailing timestamp (no prompt) or ask you to confirm.

## v8.96 — Amazon Ads Bulk Sheet ingest + diff engine (real change log; replaces v8.95 inferred model)
- **User's real problem** (from v8.95 discussion): "i think a lot of these are based on automatic rules. i don't know abou this inferred model. there isn't a report we can build?" Jason correctly pushed back on v8.95's spend-inference model — Amazon's auto-bidding produces huge daily spend swings that had NOTHING to do with him or his agency making changes. So v8.95 was mostly labeling algorithm noise as "activity."
- **Jason then provided** two real Amazon Ads Bulk Sheets (`bulk-aq2f3xtsa2z9s-20260811-*.xlsx` + `-20260905-*.xlsx`) downloaded from advertising.amazon.com/bulk-operations. Each file is a FULL SNAPSHOT of the account: 2907+ rows per file × 54 columns × 9 entity types (Product Ad · Keyword · Bidding Adjustment · Negative Keyword · Negative Product Targeting · Ad Group · Campaign · Product Targeting · Campaign Negative Keyword).
- **v8.96 approach — snapshot + diff instead of infer.** Store each Bulk Sheet upload as a full snapshot. When ≥2 snapshots exist, diff consecutive pairs by stable entity key (entity type, campaign_id, ad_group_id, keyword_id / product_targeting_id / ad_id) and emit a change event for every field that differs. Result: a REAL change log where every event reflects a decision (yours, your agency's, or a bulk rule you authored) — no algorithm noise.
- **⚠ SQL TO RUN:** `supabase_v8_96_amazon_ads_bulk.sql` — creates `amazon_ads_bulk_upload` (upload metadata) + `amazon_ads_bulk_snapshot` (per-row entity data). snapshot table has 20+ diff-relevant fields (bid, daily_budget, state, match_type, bidding_strategy, placement, percentage, product_targeting_expression, targeting_type, sku, start_date, end_date, asin). RLS enabled, authenticated policies, anon revoked.
- **New uploader tile** — 🛠 Amazon Ads — Bulk Sheet (change history) on Data → Uploads. Accepts multi-file drops so Jason can bulk-load his entire monthly history in one gesture for retroactive change detection. Auto-extracts snapshot_date from filename pattern `bulk-{advertiser}-{YYYYMMDD}-{YYYYMMDD}-{ts}.xlsx` (uses the LATER date = end of the export window = state at time of download). Only reads the "Sponsored Products Campaigns" sheet; other tabs ignored.
- **`parseAmazonAdsBulkSheet` + `handleAmazonAdsBulkUpload`** — chunked inserts (500 rows/batch) with live status line. Missing-table error routes to a clear "run the SQL migration first" message. Multi-file drops process sequentially with per-file progress (`⏳ (3/8) reading bulk-20260811.xlsx…`).
- **`loadAmazonAdsBulk()`** — paginated loader (1000 rows/page). Groups snapshot rows by upload_id into a Map for O(1) diff-engine access. Cached in `amazonAdsBulkUploads` + `amazonAdsBulkSnapshotsByUpload`.
- **Diff engine** — v8.96 rewrite of `renderPnlAdActivity`. Walks consecutive upload pairs. For each pair (prev, cur): builds an entity-key Map for each side, diffs every entity present in both against 14 tracked fields, emits ADDED for entities only in cur, emits REMOVED for entities only in prev. Field-specific event types: `bid_up` / `bid_down` / `budget_up` / `budget_down` / `paused` / `enabled` / `archived` / `state_change` / `match_change` / `strategy_change` / `portfolio_move` / `added` / `removed` / generic `change`. Money fields tolerate sub-cent noise (<$0.005 delta = no event); string fields treat null and empty string as equivalent.
- **Diff engine UI** — same shape as v8.95 (headline strip · summary chips · monthly bar chart · event log table) so the visual language is familiar. Chip counts: raised (bid_up + budget_up) · cut · paused · enabled · added · removed · match type · strategy. Event log columns: snapshot date · entity · change (label + before → after) · campaign › ad group · product (ASIN + name via `allProducts` join). Empty-state messages explicitly explain the 1-snapshot vs 2+-snapshot cases.
- **Amazon's native History page — noted for future.** Jason surfaced `advertising.amazon.com/cm/history?entityId=...` mid-turn: Amazon's own change log lives there, with 5,445+ YTD entries already accessible, filterable by change type (Bid adjustment · Campaign bidding strategy · Budget · Schedule · Status · Bid). Currently no bulk CSV export from that page and the Ads API's ChangeSets endpoint requires SP-API integration. The Bulk Sheet diff is the workable substitute until either surface becomes available. Explainer at the bottom of the timeline links out to the History page for manual cross-check.
- **Freshness probe + TABLE_TO_UPLOAD_GROUP wiring** — both `amazon_ads_bulk_upload` and `amazon_ads_bulk_snapshot` route to `grp-amazon-ads-bulk`. Uploads-page freshness card shows `📅 Latest snapshot {date} · {rows} rows · uploaded {ago}`.
...
- **Fix (v7.00)**: rewrite the delete flow as a full purge with a preview:
  - **Pre-count phase**: `countProductChildren(masterId)` runs a `select count(*) exact head:true` against every one of the 12 master_id-carrying child tables (`sales_weekly`, `sku_economics`, `sku_economics_eu`, `chewy_forecasts`, `shopify_sales_daily`, `walmart_sales_weekly`, `amazon_ad_spend`, `fba_inventory_snapshots`, `fba_shipments`, `channel_listings`, `product_cogs`, `inventory`) plus BOM tracked by both `bundle_master_id` AND `component_master_id` (separately).
  - **Confirmation dialog** now lists every table with its row count. Zero-count lines are grayed out; non-zero are bright so the operator's eye lands on what's actually going away. Shows the total at the bottom with a "Large purge — cannot be undone" note if > 100 rows.
  - **Component-in-other-bundle warning**: if the product appears as `component_master_id` in N bundles, a prominent red callout above the row list says "This product appears in N other bundles' BOM. Deleting will remove those BOM rows and leave the parent bundles missing a component. Consider merging into another product first (use the Merge tool)." Prevents the silent-orphan-bundles-BOM footgun.
  - **Purge phase**: deletes BOM rows referencing this product from BOTH columns first, then loops the 12 child tables in FK-safe order, then deletes the product row. Each delete is try/caught individually so a missing-table environment doesn't abort the whole purge. Errors accumulate into an `errors` array; if the final `products.delete` still fails, the accumulated errors are surfaced in the alert so the operator can see which child table blocked it.
  - **Partial-failure handling**: if the product row deletes but some child cleanup errored, the operator sees a warn alert with the affected tables listed. The product is gone at that point, so any leftover orphan rows can be inspected via Data → Query Database.
- **UX detail**: dialog title now reads "🗑 Delete Product + Purge All Data" (was "⚠ Delete Product") and the button label is "🗑 Delete + Purge" to match. Body copy: "This will permanently delete the product record AND every child row across every table."
- **Not offering undo for delete** (unlike merge in v6.99): full purge across 12 tables would need snapshotting arbitrarily large row sets. If Jason needs to restore, Supabase's daily backups are the answer. The pre-count preview + explicit purge affordance is the safety net.
...
8. **ALWAYS re-audit the merge tool + SP-TEMP promotion + doRestore when making code changes.** This is Jason's standing rule (v8.47). Before shipping ANY change touching schema or persistence:
   - **New table with `master_id` FK** → add to `runMerge`'s `reassignMasterId(table)` call list AND `saveProduct`'s SP-TEMP promotion FK-migration block AND `doRestore`'s per-channel logic. If FK is `ON DELETE CASCADE`, omission silently DELETES data on merge. If soft-ref, omission silently orphans rows (v4.145 sku_economics_eu, v5.44 chewy_sales_weekly, v5.76 chewy_sales_weekly reassign, v6.98 fba_inventory_snapshots + 4 others).
   - **New value-bearing column on `products`** → add to `runMerge`'s `SIMPLE_BACKFILL_FIELDS` array (or `BOOL_FLAG_FIELDS` for booleans) so a survivor with an empty value inherits from the duplicate (v5.45 caught image_url + 8 more; v5.77 caught per-region matrix).
   - **New in-memory cache written on save** → mirror to `allProducts` / `records[]` / `cogsByMaster` etc. so UI stays consistent without reload (v7.34 pattern).
   - **Current master_id-bearing tables:** sales_weekly, sku_economics, sku_economics_eu, chewy_forecasts, inventory, bom (BOTH bundle_master_id + component_master_id), product_cogs, chewy_sales_weekly, fba_inventory_snapshots, fba_shipments, walmart_sales_weekly, shopify_sales_daily, amazon_ad_spend, channel_listings. Any new table joined by master_id must be added.
   - **Grep-based verification:** search index.html for `reassignMasterId(` in `runMerge` and `.update({ master_id })` / `.eq('master_id', oldMasterId)` in `saveProduct`'s SP-TEMP promotion. If your new table isn't in that list, add it.
   - **Silent data loss has bitten this 6+ times (v4.145, v5.44, v5.45, v5.76, v6.98, v7.00, v7.34).** Merges that "worked" but dropped data are the worst class of bug in this codebase — deletes cascade before the operator can notice. Belt-and-suspenders: re-audit even when the diff looks small.
